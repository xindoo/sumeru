...
worldbuilder 在协调所有涉及章节级操作的子技能时，强制执行以下规则：

**每个子Agent最多负责3个章节**（硬性约束，不可违反）

此规则适用于以下所有阶段：
- **写作阶段**（sumeru-write）：批量生成章节时，每个子Agent最多写3章
- **审查阶段**（sumeru-review）：批量审查章节时，每个子Agent最多审查3章；修复阶段同理
- **润色阶段**（sumeru-polish）：批量润色章节时，每个子Agent最多润色3章
- **完稿阶段**（sumeru-finalize）：批量校验/导出时，每个子Agent最多处理3章

**设计原因**：
- 防止单个Agent处理过多章节导致上下文溢出、质量下降
- 确保每个章节都能获得足够的生成/校验/润色质量
- 3章是兼顾效率和质量的平衡点——足够保持章节间的上下文连贯性，又不会因任务过重而降低产出质量
...
[sumeru-review] 逻辑审查（**审查所有章节，每个Agent最多3章**）
    ...
    → ✅ **必须完成全本审查**
    → ⚠️ **每个子Agent最多负责3个章节的审查和修复**
    ↓
[阶段检查点 3] 验证审查完成，记录审查问题
    ↓
{审查问题是否需要修复?}
    →|是| [定位问题章节 → 调用sumeru-write修复 → 替换chapters/下的原文件]
    →|否| 继续
    ↓
[sumeru-polish] 内容润色（**润色所有章节，每个Agent最多3章**）
    ...
    → ✅ **必须完成全本润色**
    → 📝 **润色结果自动替换到chapters/目录下**
    → ⚠️ **每个子Agent最多负责3个章节的润色**
    ↓
[阶段检查点 4] 验证润色完成
    ↓
...
- **审查阶段**：确认 `剧情审查报告.md` 生成且包含全本审查结果。如有修复，检查 `chapters/` 目录下的原文件是否已被修复版本替换
- **润色阶段**：确认 `.sumeru/polish/modified/` 目录下润色章节数与原章节数一致，并检查 `chapters/` 目录下的文件已被润色版本替换
...
flowchart LR
    ...
    H --> I[修复结果替换到chapters/]
    I --> F
    G -->|否| J[调用 polish 全本内容润色]
    J --> K[润色结果替换到chapters/]
    K --> L{润色确认?}
    L -->|否| M[针对性调整润色]
    M --> J
    L -->|是| N[调用 finalize 完稿校验]
    N --> O[生成创作报告与作品文件]
    
    subgraph 进度保存
        P[WORKBUILDER_PROGRESS.json]
        E -->|保存进度| P
        F -->|保存进度| P
        K -->|保存进度| P
        N -->|保存进度| P
...
5. **创作阶段**：按细纲驱动批量创作所有章节，支持并行生成，**必须完成所有章节**后才进入下一阶段，**每个子Agent最多负责3个章节**，完成后更新进度文件
6. **审查阶段**：调用 `sumeru-review --all` 执行三阶段审查修复流程，**每个子Agent最多负责3个章节**：
    - **第一阶段：全局审查**：分析整体剧情脉络、时间线、设定一致性、冲突点分布、伏笔回收状态
   - **第二阶段：章节细节审查**：逐章检查字数、时间线、人物OOC、物品状态、场景质量、伏笔设置
   - **第三阶段：统一修复**（调用write重写修复）：
     - 合并全局和章节问题，按严重程度统一修复
     - 对于需要重写的章节，调用 `sumeru-write` 技能进行重写
     - 确保修复后的内容符合剧情逻辑和人物性格
   - **字数检查与填充**：在第二阶段逐章统计字数，对不足的章节自动填充内容（强化场景描写、丰富对话、补充心理活动等）
   - **自动修复所有问题**：不只是提出问题，而是自动修复所有检测到的问题
   - 修复结果**直接替换到 chapters/ 目录下**的原文件
   - 修复完成后重新验证，确保所有问题已解决
   - 完成后更新进度文件
7. **润色阶段**：调用 `sumeru-polish --all` 进行全本内容润色，**每个子Agent最多负责3个章节**，提供多种润色风格选项（精简/详写/抒情/热血等）
   - 润色完成后，**将修改后的章节内容自动替换到 chapters/ 目录下**
   - 润色过程和结果记录到 `.sumeru/polish/` 目录
   - 完成后更新进度文件
8. **完稿阶段**：调用 `sumeru-finalize` 对所有润色后的章节进行完稿校验，**每个子Agent最多负责3个章节**，从 `chapters/` 目录读取最终内容（已包含review修复和polish润色后的结果），输出多种格式（Markdown/HTML/EPUB）到 `publish/` 目录，生成创作总结报告，完成后更新进度文件为最终完成状态
...
4. **断点恢复**：使用 `--resume` 参数时自动从 `WORKBUILDER_PROGRESS.json` 读取上次进度，从未完成阶段继续
...
```json
{
  ...
  "stages": {
    ...
    "review": {
      ...
      "chapters_repaired": [
        "chapters/第15章_误会加深.md",
        "chapters/第42章_时间线冲突.md"
      ],
      "chapters_replaced": true
    },
    "polish": {
      "status": "in_progress",
