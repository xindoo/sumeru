---
name: sumeru-polish
description: 小说内容润色优化，适用于用户说"帮我润色这段小说"、"改下文笔"、"优化章节节奏"、"强化这个爽点"、"让对话更自然"、"把这段写得更爽"、"优化小说文笔"、"调整章节节奏"、"让对话更真实"、"帮我改下这段内容"、"润色小说"、"优化爽点"、"提升文笔"、"让这段更有代入感"等需求，提供3级润色等级，专注文笔与内容层面优化，支持风格适配、节奏收紧、爽点强化、对话优化等专项优化，**润色结果保存到 staging 区域供用户确认后再应用**，**批量润色时使用子Agent并行处理，每个Agent最多负责3个章节**
type: skill
---
...
- 实时显示已完成/进行中/待润色章节状态
- 所有Agent完成后，润色结果保存到 `.sumeru/polish/modified/`。使用 `--apply` 参数可自动将润色结果应用到 `chapters/` 目录，替换原文件

### 核心参数
| 参数 | 类型 | 说明 | 示例 |
|------|------|------|------|
| `--level` | number | 润色级别：1(轻度)/2(中度)/3(深度) | `--level 2` |
| `--deep` | boolean | 深度润色模式 | `--deep` |
| `--style` | string | 目标风格 | `--style 小白爽文` |
| `--focus` | string | 优化重点，逗号分隔 | `--focus 爽点强化,节奏收紧` |
| `--apply` | boolean | 将润色结果从 `.sumeru/polish/modified/` 应用到 `chapters/` 目录（默认：false） | `--apply` |
...
- **后续 Skill**：润色后的内容供 `sumeru-finalize` 使用
  - 默认情况下润色结果保存在 `.sumeru/polish/modified/`，不影响 `chapters/` 原文件
  - worldbuilder 调用 polish 时自动带上 `--apply` 参数，将润色结果应用到 `chapters/` 目录
