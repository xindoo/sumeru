...
- 这是为了保证每个章节的生成质量，避免单个Agent处理过多章节导致上下文溢出和内容质量下降
- 章节分配示例：100章任务 → 至少需要 ceil(100/3)=34个子Agent并行处理
- 调度器自动计算所需Agent数量，无需手动指定
...
**章节分配规则**
- 每个子Agent最多负责3个章节（硬性约束，不可超过）
- 分配策略：按章节顺序连续分配，如Agent1负责第1-3章，Agent2负责第4-6章，以此类推
