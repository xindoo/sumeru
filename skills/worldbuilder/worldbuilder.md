...
1. **选题策划**：调用 `topic` 进行市场分析、选题定位，生成核心创意与卖点
2. **大纲设计**：调用 `outline` 构建完整世界观、人物设定、分卷大纲与章节规划
3. **内容创作**：调用 `write` 按大纲进行分章节内容撰写，保持风格统一
4. **逻辑审查**：调用 `review` 校验时间线一致性、人物行为逻辑、前后剧情连贯性
5. **内容润色**：调用 `polish` 进行文笔优化、细节丰满、节奏调整
6. **完稿校验**：调用 `final` 完成错别字检查、标点规范、逻辑漏洞最终排查与作品打包
